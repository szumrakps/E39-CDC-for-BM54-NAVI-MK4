...
// --- SKRÓCONA I ZABEZPIECZONA MASZYNA STANÓW TEKSTU ---
    ...
    if (textState != TEXT_IDLE) { 
        switch (textState) {
            case TEXT_INIT:
                // Skrócono czas oczekiwania do 30 ms
                if (millis() - textTime >= 30) {
                    // Krytyczny warunek: upewniamy się, że i-BUS na pewno milczy!
                    if (!busIdle()) {
                        textTime = millis(); // Linia zajęta? Przesuwamy próbę o kolejne 30 ms
                        break;   
                    }
                    sendArtist(); 
                    if (!isCdcActive) {
            ...
            case TEXT_SEND_LINE1:
                // Skrócono czas oczekiwania do 30 ms
                if (millis() - textTime >= 30) {
                    // Ponowne sprawdzenie zajętości linii przed tytułem
                    if (!busIdle()) {
                        textTime = millis(); // Czekamy na wolną linię
                        break;   
                    }
                    sendText(); 
            ...
            case TEXT_SEND_LINE2:
                // Skrócono czas oczekiwania do 30 ms
                if (millis() - textTime >= 30) {
                    // Ponowne sprawdzenie zajętości linii przed ramką kończącą
                    if (!busIdle()) {
                        textTime = millis(); // Czekamy na wolną linię
                        break;   
                    }
...
void sendText() {
    ...
    uint8_t f[64]; // Zwiększona pojemność tablicy ramki ze 40 do 64
...
void sendEndFrame() {
    if (titleOffset > 0 && titleOffset < strlen(currentTitle)) {
        ...
        // DRUGA LINIA RÓWNIEŻ DOSTAJE DO 30 ZNAKÓW
        uint8_t remLen = strlen(currentTitle) - titleOffset;
        if (remLen > 30) remLen = 30; 

        uint8_t f[64]; // Bezpieczny rozmiar tablicy
...
void sendArtist() {
    ...
    if (len > 30) len = 30; // Wykonawca też może zająć do 30 znaków
